...
RangeFinder::RangeFinder(uint32_t inMaxFramesPerSlice, uint32_t inNumFreq, float inStartFreq,
    ...
    for (uint32_t i = 0; i < MAX_NUM_FREQS; i++) {
        ...
        for (uint32_t k = 0; k < CIC_SEC; k++) {
            mCICBuffer[i][k][0] = (float *) calloc(mRecDataSize / CIC_DEC + CIC_DELAY,
                                                   sizeof(float));
            mCICBuffer[i][k][1] = (float *) calloc(mRecDataSize / CIC_DEC + CIC_DELAY,
                                                   sizeof(float));
        }
    }
...
//    DebugLog("%f %f %f",mFRecDataBuffer[0],mFRecDataBuffer[1],mFRecDataBuffer[2]);
    ...
    {
        DSP_vmul(mFRecDataBuffer, mCosBuffer[i] + mCurProcPos, mTempBuffer,
                 mCurRecPos); //multiply the cos
        cid = 0;
        //sum CIC_DEC points of data, put into CICbuffer
        memmove(mCICBuffer[i][0][cid], mCICBuffer[i][0][cid] + mLastCICPos,
                CIC_DELAY * sizeof(float));
        index = CIC_DELAY;
        ...
        DSP_vmul(mFRecDataBuffer, mSinBuffer[i] + mCurProcPos, mTempBuffer,
                 mCurRecPos); //multiply the sin
        cid = 1;
        //sum CIC_DEC points of data, put into CICbuffer
        memmove(mCICBuffer[i][0][cid], mCICBuffer[i][0][cid] + mLastCICPos,
                CIC_DELAY * sizeof(float));
...
float RangeFinder::CalculateDistance() {
    ...
    for (int f = 0; f < mNumFreqs; f++) {
        ...
        temp_val = 0;
        for (int i = 0; i < mDecsize; ++i) {
            tempdata[i] = tempcomplex.realp[i] * tempcomplex.realp[i] +
                          tempcomplex.imagp[i] * tempcomplex.imagp[i];
            temp_val += tempdata[i];
        }
...
//        DebugLog("numfrequesd = 0");
    ...
    float deltax = deltax = mNumFreqs * ((mDecsize - 1) * mDecsize * (2 * mDecsize - 1) / 6 -
                                         (mDecsize - 1) * mDecsize * (mDecsize - 1) / 4);
    float delta = (float) ((sumxy - sumy * (mDecsize - 1) / 2.0) / deltax * mNumFreqs /
                           numfreqused);
